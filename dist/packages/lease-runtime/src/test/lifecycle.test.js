...
import { deriveLeaseHealthState, selectStartupAction, shouldReacquireAfterLookupError, } from '../leaseLifecycle.js';
import { deniedReason, healthParityCases, startupParityCases } from './parityMatrix.js';
function buildLeaseStatus(overrides = {}) {
    return {
        lease_id: 'lease-1',
...
test('shared health parity cases stay aligned', () => {
    for (const scenario of healthParityCases) {
        assert.equal(deriveLeaseHealthState(scenario.leaseStatus, new Date('2026-03-23T00:00:00.000Z')), scenario.expectedHealth, scenario.name);
    }
});
