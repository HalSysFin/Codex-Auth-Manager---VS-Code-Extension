...
test('acquire plus materialize returns a writable auth payload', async () => {
    ...
    const prepared = prepareAuthPayloadForWrite(materialize.credential_material.auth_json, '2026-03-23T00:05:00.000Z');
...
test('backend denial preserves no eligible credentials reason', async () => {
    ...
    await assert.rejects(() => client.acquireLease({
        machineId: 'machine-a',
        agentId: 'agent-a',
    }), (error) => error instanceof AuthManagerClientError &&
        error.status === 409 &&
...
test('backend unavailable errors surface without fake success payloads', async () => {
    ...
    await assert.rejects(() => client.getLease('lease-1'), /ECONNREFUSED/);
});
//# sourceMappingURL=clientFlow.test.js.map